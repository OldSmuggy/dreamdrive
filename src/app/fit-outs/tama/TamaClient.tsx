    ...
    <div className="min-h-screen">
      ...
      <FitoutHero fallbackImage={STATIC_HERO} heroImage={content.hero_image} heroVideo={content.hero_video}>
        ...
        <div className="max-w-2xl">
          ...
          <a href="tel:0432182892" className="inline-block bg-sand-400 text-forest-950 font-semibold px-8 py-3.5 rounded-xl hover:bg-sand-300 transition-colors text-base">
            Book a Test Drive
          </a>
        </div>
      </FitoutHero>
            ...
            {[
              ...
              <div key={c.title} className="bg-white rounded-2xl border border-gray-200 p-7">
                <div className="w-10 h-10 bg-forest-100 rounded-xl flex items-center justify-center mb-4">
                  <svg className="w-5 h-5 text-forest-700" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>
                </div>
            ...
            <ul className="space-y-3 text-sm text-gray-700">
              {['2WD: 2.0L petrol engine', '4x4 upgrade: 2.8L Turbo Diesel (+$7,000)', 'Same engine as Land Cruiser Prado', '12% shorter exterior, 10% more rear cabin space vs competitors', '2026 model H200 still available from our Japan factory'].map(item => (
                <li key={item} className="flex items-start gap-2.5">
                  <svg className="w-4 h-4 text-forest-600 mt-0.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>
                  {item}
                </li>
              ))}
            </ul>
          ...
          <div>
            ...
            <ul className="space-y-2.5 text-sm text-gray-700">
              {['2000W inverter', '200AH Lithium battery', 'D/C Charger (charges from driving & shore power)', 'Shore Power Charger', 'LED Down Lights + Dimmable LED Light Bar', 'A/C Charging Outlets x2 with USB Type A & C'].map(item => (
                <li key={item} className="flex items-start gap-2.5"><span className="text-forest-600 shrink-0 mt-0.5">⚡</span>{item}</li>
              ))}
            </ul>
              ...
              <div>
                ...
                <ul className="space-y-1.5 text-sm text-gray-700">
                  {['3 transforming rear seats', 'Modular bed kit', '40L fridge', 'Walnut countertop, sink & faucet', '10L fresh water tank (optional 40L)', 'Portable table', 'Handcrafted furniture', 'High pressure pump', 'Quick release shower hose'].map(i => (
                    <li key={i} className="flex items-center gap-2"><span className="w-1.5 h-1.5 bg-forest-500 rounded-full shrink-0" />{i}</li>
                  ))}
                </ul>
              </div>
              <div>
      ...
      <section className="bg-forest-950 text-white py-16 px-4">
        <div className="max-w-2xl mx-auto text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <Link href="/build" className="bg-sand-400 text-forest-950 font-semibold px-8 py-3.5 rounded-xl hover:bg-sand-300 transition-colors text-base text-center">Configure Your Build →</Link>

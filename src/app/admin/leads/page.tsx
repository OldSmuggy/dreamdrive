  ...
  return (
    <div>
      <div className="flex items-baseline justify-between mb-6">
        <h1 className="text-3xl text-charcoal">Leads</h1>
          ...
          <Link
            ...
            className={`px-4 py-1.5 rounded-full text-sm font-medium border transition-colors ${
              activeTab === tab.value
                ...
                : 'bg-white text-gray-600 border-gray-300 hover:border-ocean'
            }`}
          >
            {tab.label}
                ...
                <div className="flex items-start justify-between flex-wrap gap-2">
                  <div>
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span className={`text-xs font-bold px-2 py-0.5 rounded ${
                        lead.status === 'new' ? 'bg-ocean text-white' : 'bg-gray-200 text-gray-600'
                      }`}>{lead.status}</span>
                      {isDeposit && (
                    ...
                    <div className="flex gap-4 mt-1.5 text-sm text-gray-600 flex-wrap">
                      {lead.email && <a href={`mailto:${lead.email}`} className="hover:text-ocean">{lead.email}</a>}
                      {lead.phone && <a href={`tel:${lead.phone}`} className="hover:text-ocean">{lead.phone}</a>}
                    ...
                    {lead.build_slug && (
                      <a
                        ...
                        className="inline-flex items-center gap-1 mt-1.5 text-xs text-ocean hover:underline font-medium"
                      >
                        View Build →
                      </a>
                    )}
                  ...
                  <div className="text-right text-sm shrink-0">
                    {lead.estimated_value && (
                      <p className="font-semibold text-ocean">{centsToAud(lead.estimated_value)}</p>
                    )}

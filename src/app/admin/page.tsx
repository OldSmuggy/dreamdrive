...
export default function AdminImportPage() {
  ...
  const handleImport = async () => {
    ...
    if (!sessionCookie.trim()) {
      alert('Paste your session cookie first')
      return
    }
  ...
  return (
    <div className="max-w-3xl">
      <div className="mb-6">
        <h1 className="text-3xl text-charcoal">Import from NINJA</h1>
        <p className="text-gray-500 text-sm mt-1">
          Paste vehicle URLs from ninja-cartrade.jp to import listings directly
        </p>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6 mb-5">
        <div className="flex items-start gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">This lets the import tool log in as you. You only need to do this once per session.</p>
          </div>
        </div>
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 mb-4 text-sm text-amber-900">
          ...
          <ol className="list-decimal list-inside space-y-1 text-amber-800">
            ...
            <li>Right-click anywhere → <strong>Inspect</strong></li>
            <li>Click the <strong>Network</strong> tab</li>
            <li>Refresh the page (F5)</li>
            <li>Click the first request in the list</li>
            <li>Scroll right panel to <strong>Request Headers</strong></li>
            <li>Find the line starting with <strong>cookie:</strong> and copy the whole value</li>
            <li>Paste it below</li>
          </ol>
        </div>

        <button onClick={handleLoadCookie} className="text-xs text-ocean underline mb-2 block">
          Load saved cookie
        </button>
        ...
        <div className="flex justify-end mt-2">
          <button onClick={handleSaveCookie} className="text-sm px-4 py-1.5 bg-ocean text-white rounded-lg hover:bg-ocean">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6 mb-5">
        <div className="flex items-start gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">One URL per line. Copy from the browser address bar on a vehicle detail page.</p>
          </div>
        </div>

        <div className="bg-gray-50 border border-gray-200 rounded-lg p-3 mb-3 text-xs font-mono text-gray-500">
          https://www.ninja-cartrade.jp/ninja/cardetail.action?KaijoCode=TK&AuctionCount=1557&BidNo=40261
        </div>
        ...
        <div className="space-y-3">
            ...
            <div key={i} className={`rounded-xl border p-4 ${r.success ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
              {r.success && r.data ? (
                <div className="flex gap-4 items-start">
                  {r.data.photos?.[0] && (
                    <img src={r.data.photos[0]} alt="" className="w-24 h-16 object-cover rounded-lg shrink-0" />
                  )}
          ...
          {results.every(r => r.success) && (
            <div className="text-center pt-2">
              <a href="/admin/listings" className="text-ocean text-sm font-semibold underline">→ View all listings</a>
            </div>
          )}
        </div>
      )}

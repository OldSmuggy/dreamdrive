...
const STAGE_ORDER = [
  ...
  'van_building', 'shipping', 'compliance',
...
function StageTracker({
  ...
  const visibleStages = hasPoptop
    ? STAGE_ORDER
    : STAGE_ORDER.filter(s => s !== 'pop_top_install')
  ...
  const currentIdx     = currentStage ? visibleStages.indexOf(currentStage.stage) : -1
  const canAdvance     = currentIdx < visibleStages.length - 1
  ...
  const advance = async (direction: 'advance' | 'back') => {
    ...
    const res = await fetch(`/api/customers/${customerId}/vehicles/${vehicleId}/stages`, {
      ...
      body: JSON.stringify({ action: direction, skip_poptop: !hasPoptop }),
    })
    ...
    <div className="space-y-3">
          ...
          {sortedStages.map((stage, i) => {
            ...
            return (
              <div key={stage.stage} className="flex items-start">
                <button
                  onClick={() => {
                    if (done || active) setExpandedStage(expandedStage === stage.stage ? null : stage.stage)
                    else if (stage.status === 'upcoming') jumpTo(stage.stage)
                  }}
                  title={stage.status === 'upcoming' ? `Jump to ${STAGE_LABELS[stage.stage]}` : 'Click to expand'}
                ...
                >
                  <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold border-2 transition-colors
                    ...
                    ${!done && !active ? 'bg-gray-100 border-gray-300 text-gray-400 hover:border-gray-400': ''}
                  `}>
                    {done ? (
                  ...
                  {done && stage.completed_at && (
                    <p className="text-[9px] text-gray-400 whitespace-nowrap">{fmtDate(stage.completed_at)}</p>
                  )}
                  {active && stage.entered_at && (
                    <p className="text-[9px] text-forest-400 whitespace-nowrap">{fmtDate(stage.entered_at)}</p>
                  )}
                </button>
      ...
      {expandedStage && (() => {
        ...
        return (
          <div className="bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-xs space-y-1">
            ...
            {s.entered_at   && <p className="text-gray-500">Entered: {fmtDate(s.entered_at)}</p>}
            {s.completed_at && <p className="text-gray-500">Completed: {fmtDate(s.completed_at)}</p>}
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex-1 text-center">
          ...
          <span className="text-xs text-gray-400 ml-1">({currentIdx + 1}/{visibleStages.length})</span>
        </div>
        <button
...
function BuildEditor({
  ...
  const addonTotal = form.addon_slugs.reduce((sum, slug) => {
    const p = addonProducts.find(ap => ap.slug === slug)
  ...
  const calcTotal = () => {
    ...
    return conv + poptop + addonTotal + custom
  }
  ...
  return (
    <div className="space-y-3 pt-2">
      ...
      {form.build_type !== 'none' && form.build_type !== 'custom' && (
        <div>
          ...
          <div className="relative">
            ...
            <input
              type="number"
              value={form.conversion_fee_aud}
              onChange={e => setForm(f => ({ ...f, conversion_fee_aud: e.target.value }))}
              className="w-full border border-gray-300 rounded-lg pl-7 pr-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-forest-500"
              placeholder="0"
            />
          </div>
        </div>
        ...
        <div className="space-y-2">
          <div>
            ...
            <textarea
              ...
              rows={2}
              className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-forest-500"
            />
          </div>
          <div>
            ...
            <div className="relative">
              ...
              <input
                type="number"
                value={form.custom_quote_aud}
                onChange={e => setForm(f => ({ ...f, custom_quote_aud: e.target.value }))}
                className="w-full border border-gray-300 rounded-lg pl-7 pr-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-forest-500"
              />
            </div>
          </div>
      ...
      {form.pop_top && (
        <div>
          ...
          <div className="relative">
            ...
            <input
              type="number"
              value={form.pop_top_fee_aud}
              onChange={e => setForm(f => ({ ...f, pop_top_fee_aud: e.target.value }))}
              className="w-full border border-gray-300 rounded-lg pl-7 pr-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-forest-500"
            />
          </div>
        </div>
      ...
      {addonProducts.length > 0 && (
        <div>
          <label className="block text-xs font-semibold text-gray-600 mb-2">Add-ons</label>
      ...
      <div className="grid grid-cols-2 gap-3 pt-1">
        <div>
          ...
          <div className="relative">
            ...
            <input
              type="number"
              value={form.total_quoted_aud}
              onChange={e => setForm(f => ({ ...f, total_quoted_aud: e.target.value }))}
              placeholder={estimatedTotal > 0 ? String(estimatedTotal / 100) : '0'}
              className="w-full border border-gray-300 rounded-lg pl-7 pr-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-forest-500"
            />
          </div>
          {estimatedTotal > 0 && (
            <p className="text-xs text-gray-400 mt-0.5">Auto-calculated: {centsToAud(estimatedTotal)}</p>
          )}
        </div>
        <div>
          ...
          <select
            ...
            onChange={e => setForm(f => ({ ...f, build_status: e.target.value }))}
            className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-forest-500"
          >
      ...
      <button
        ...
        className={`text-xs px-4 py-2 rounded-lg font-medium transition-colors disabled:opacity-50 ${
          saved ? 'bg-green-600 text-white' : 'bg-forest-600 text-white hover:bg-forest-700'
        }`}
      >
          ...
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="min-w-0">
              <button
                ...
                className="text-sm font-medium text-forest-700 hover:underline truncate block"
              >
                {doc.name}
              </button>
        ...
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-3 space-y-2">
          <div>
            ...
            <input
              ...
              className="w-full text-xs text-gray-600 file:mr-2 file:py-1 file:px-3 file:rounded-lg file:border-0 file:text-xs file:bg-forest-600 file:text-white hover:file:bg-forest-700"
            />
          </div>
          <div className="grid grid-cols-2 gap-2">
            <div>
              ...
              <input
                ...
                className="w-full border border-gray-300 rounded-lg px-2 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-forest-500"
              />
            </div>
            <div>
              ...
              <select
                ...
                className="w-full border border-gray-300 rounded-lg px-2 py-1.5 text-xs bg-white focus:outline-none focus:ring-2 focus:ring-forest-500"
              >
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => { setShowUpload(false); setSelectedFile(null); setUploadErr(null) }} className="text-xs px-3 py-1.5 border border-gray-300 rounded-lg text-gray-600 hover:bg-gray-50">
              Cancel
            </button>
          </div>
        </div>
...
function VehicleCard({
  ...
  const statusLabel: Record<string, string> = {
    searching: 'Searching', targeted: 'Targeted', bidding: 'Bidding',
    purchase: 'Purchase', van_building: 'Van Building', shipping: 'Shipping',
    compliance: 'Compliance', pop_top_install: 'Pop Top Install',
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${statusColors[vehicle.vehicle_status] ?? 'bg-gray-100 text-gray-600'}`}>
                {statusLabel[vehicle.vehicle_status] ?? vehicle.vehicle_status}
              </span>
            </div>
            {vehicle.listing && (
      ...
      <div className="px-4 py-4 space-y-5">
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            {vehicle.listing ? (
              ...
              <button
                onClick={() => setShowListingSearch(v => !v)}
                className="text-xs px-2.5 py-1 bg-forest-600 text-white rounded-lg hover:bg-forest-700"
              >
          ...
          {vehicle.listing ? (
            <div className="flex items-center gap-3 bg-gray-50 rounded-lg p-2">
              {vehicle.listing.photos?.[0] && (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={vehicle.listing.photos[0]} alt="" className="w-20 h-14 object-cover rounded shrink-0" />
              )}
          ...
          {showListingSearch && (
            <div className="mt-2 relative">
              <input
                ...
                placeholder="Search by model, chassis, bid no…"
                ...
                <div className="absolute z-20 w-full bg-white border border-gray-200 rounded-lg shadow-lg mt-1 max-h-52 overflow-y-auto">
                  {listingResults.map(l => (
                    <button
                      key={l.id}
                      type="button"
                      onClick={() => linkListing(l)}
                      disabled={linkingListing}
                      className="w-full text-left px-3 py-2 hover:bg-gray-50 text-sm flex items-center gap-2 disabled:opacity-50"
                    >
              ...
              <div className="mt-1 text-right">
                <Link href="/admin/listings" target="_blank" className="text-xs text-forest-600 hover:underline">
                  Browse all listings →
                </Link>
              </div>
            </div>
        ...
        {/* ── Target Preferences (only if no listing) ── */}
        {!vehicle.listing && (
          <div>
            <button
              onClick={() => setShowTargetPrefs(v => !v)}
              className="flex items-center gap-2 text-xs font-semibold text-gray-600 uppercase tracking-wide mb-2 hover:text-gray-800"
            >
              Target Preferences
              <span className="text-gray-400 font-normal normal-case">{showTargetPrefs ? '▲' : '▼'}</span>
            </button>
            {showTargetPrefs && (
              <div className="space-y-3">
                <div className="grid grid-cols-3 gap-2">
                  <div>
                    ...
                    <input type="number" value={prefs.min_year ?? ''} onChange={e => setPrefs(p => ({ ...p, min_year: e.target.value }))} className="w-full border border-gray-300 rounded-lg px-2 py-1.5 text-xs" placeholder="2020" />
                  </div>
                  <div>
                    <label className="block text-xs text-gray-500 mb-1">Max Year</label>
                    <input type="number" value={prefs.max_year ?? ''} onChange={e => setPrefs(p => ({ ...p, max_year: e.target.value }))} className="w-full border border-gray-300 rounded-lg px-2 py-1.5 text-xs" />
                  </div>
                  <div>
                    <label className="block text-xs text-gray-500 mb-1">Max Budget</label>
                    <div className="relative">
                      <span className="absolute left-2 top-1.5 text-xs text-gray-400">$</span>
                      <input type="number" value={prefs.max_budget_aud ?? ''} onChange={e => setPrefs(p => ({ ...p, max_budget_aud: e.target.value }))} className="w-full border border-gray-300 rounded-lg pl-5 pr-2 py-1.5 text-xs" />
                    </div>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <select value={prefs.drive ?? ''} onChange={e => setPrefs(p => ({ ...p, drive: e.target.value }))} className="w-full border border-gray-300 rounded-lg px-2 py-1.5 text-xs bg-white">
                  ...
                  <div>
                    ...
                    <select value={prefs.engine ?? ''} onChange={e => setPrefs(p => ({ ...p, engine: e.target.value }))} className="w-full border border-gray-300 rounded-lg px-2 py-1.5 text-xs bg-white">
                      <option value="">Either</option>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="block text-xs text-gray-500 mb-1">Grade Preference</label>
                    <input type="text" value={prefs.grade ?? ''} onChange={e => setPrefs(p => ({ ...p, grade: e.target.value }))} className="w-full border border-gray-300 rounded-lg px-2 py-1.5 text-xs" placeholder="Super GL" />
                  </div>
                  <div>
                    <label className="block text-xs text-gray-500 mb-1">Colour</label>
                    <input type="text" value={prefs.colour ?? ''} onChange={e => setPrefs(p => ({ ...p, colour: e.target.value }))} className="w-full border border-gray-300 rounded-lg px-2 py-1.5 text-xs" placeholder="White or Silver" />
                  </div>
                </div>
                <div>
                  <label className="block text-xs text-gray-500 mb-1">Notes</label>
                  <textarea value={prefs.notes ?? ''} onChange={e => setPrefs(p => ({ ...p, notes: e.target.value }))} rows={2} className="w-full border border-gray-300 rounded-lg px-2 py-1.5 text-xs resize-none" />
                </div>
                <button onClick={savePrefs} disabled={savingPrefs} className="text-xs px-3 py-1.5 bg-forest-600 text-white rounded-lg hover:bg-forest-700 disabled:opacity-50">
        ...
        {/* ── Purchase Pricing ── */}
        <div>
          <p className="text-xs font-semibold text-gray-600 uppercase tracking-wide mb-2">Purchase Pricing</p>
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="block text-xs text-gray-500 mb-1">Purchase Price (JPY)</label>
              <div className="relative">
                <span className="absolute left-2 top-1.5 text-xs text-gray-400">¥</span>
                <input type="number" value={purchaseJpy} onChange={e => setPurchaseJpy(e.target.value)} className="w-full border border-gray-300 rounded-lg pl-5 pr-2 py-1.5 text-xs" placeholder="0" />
              </div>
            </div>
            <div>
              <label className="block text-xs text-gray-500 mb-1">Purchase Price (AUD)</label>
              <div className="relative">
                <span className="absolute left-2 top-1.5 text-xs text-gray-400">$</span>
                <input type="number" value={purchaseAud} onChange={e => setPurchaseAud(e.target.value)} className="w-full border border-gray-300 rounded-lg pl-5 pr-2 py-1.5 text-xs" placeholder="0" />
              </div>
            </div>
          </div>
          <button onClick={savePurchasePricing} disabled={savingPurchase} className="mt-2 text-xs px-3 py-1.5 border border-gray-300 rounded-lg text-gray-600 hover:bg-gray-50 disabled:opacity-50">
            {savingPurchase ? 'Saving…' : 'Save Pricing'}
          </button>
        </div>

        {/* ── Stage Tracker ── */}
        <div>
          <p className="text-xs font-semibold text-gray-600 uppercase tracking-wide mb-3">Stage Tracker</p>
          <StageTracker
        ...
        <div>
          <button
            ...
            className="flex items-center gap-2 text-xs font-semibold text-gray-600 uppercase tracking-wide mb-1 hover:text-gray-800 w-full text-left"
          >
        ...
        <div>
          <button
            ...
            className="flex items-center gap-2 text-xs font-semibold text-gray-600 uppercase tracking-wide mb-2 hover:text-gray-800"
          >
            Documents ({docsForVehicle.length})
            <span className="text-gray-400 font-normal">{showDocs ? '▲' : '▼'}</span>
          </button>
          {showDocs && (
            <DocSection
              docs={docsForVehicle}
        ...
        <div>
          ...
          <textarea
            ...
            className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-forest-500"
          />
          <button onClick={saveNotes} disabled={savingNotes} className="mt-1.5 text-xs px-3 py-1.5 border border-gray-300 rounded-lg text-gray-600 hover:bg-gray-50 disabled:opacity-50">
...
export default function CustomerDetailClient({
  ...
  const addVehicle = async (e: React.FormEvent) => {
    ...
    const res = await fetch(`/api/customers/${customer.id}/vehicles`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ vehicle_description: addVehicleDesc || null, notes: addVehicleNotes || null }),
    })
    const data = await res.json()
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div className="w-11 h-11 rounded-full bg-forest-100 text-forest-700 flex items-center justify-center text-lg font-bold shrink-0">
            {customer.first_name[0].toUpperCase()}
          </div>
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <h1 className="font-display text-xl text-forest-900 leading-tight">{fullName}</h1>
              <span className={`text-xs px-2 py-0.5 rounded-full font-medium capitalize ${statusBadge[customer.status] ?? 'bg-gray-100 text-gray-600'}`}>
                {customer.status}
              </span>
            </div>
        ...
        <Link href={`/admin/customers/${customer.id}/edit`} className="text-sm px-3 py-1.5 border border-gray-300 rounded-lg text-gray-600 hover:bg-gray-50 shrink-0">
          Edit
        </Link>
      </div>
      ...
      {(customer.notes || customer.hubspot_contact_id) && (
        <div className="bg-white border border-gray-200 rounded-xl px-4 py-3 grid sm:grid-cols-2 gap-3">
          ...
          {customer.hubspot_contact_id && (
            <div>
              <p className="text-xs font-semibold text-gray-500 mb-1">HubSpot Contact ID</p>
              <p className="text-sm text-gray-700 font-mono">{customer.hubspot_contact_id}</p>
            </div>
          )}
        </div>
      )}
        ...
        {showAddVehicle && (
          <form onSubmit={addVehicle} className="bg-white border border-gray-200 rounded-xl p-4 mb-4 space-y-3">
            ...
            <div>
              ...
              <input
                type="text"
                value={addVehicleDesc}
                onChange={e => setAddVehicleDesc(e.target.value)}
                placeholder="e.g. 2022 Super GL 4WD Diesel — searching"
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-forest-500"
              />
            </div>
            <div>
        ...
        <div className="space-y-4">
          {vehicles.map(vehicle => (
            <VehicleCard
              ...
              onVehicleStatusUpdate={(vid, status) =>
                setVehicles(vs => vs.map(v => v.id === vid ? { ...v, vehicle_status: status } : v))
              }
      ...
      {/* General documents (not tied to a vehicle) */}
      <div className="bg-white border border-gray-200 rounded-xl px-4 py-4">
        ...
        <DocSection
          ...
          customerId={customer.id}
          vehicleId={null}
          onAdded={doc => setDocs(d => [doc, ...d])}
          onDeleted={docId => setDocs(d => d.filter(doc => doc.id !== docId))}
        />
      </div>
